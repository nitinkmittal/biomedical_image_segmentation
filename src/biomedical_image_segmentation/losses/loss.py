...
def MSELoss(
    weights: torch.Tensor, pred: torch.Tensor, target: torch.Tensor
) -> torch.tensor:
    """
    ...
    """
    weights = torch.where(target < 1.0, weights[0], weights[1])
...
def BCELoss(
    ...
    return (
        -(
            weights
            * (
                target * torch.log(pred + 1e-9)
                + (1 - target) * torch.log(1 - pred + 1e-9)
            )
        ).sum()
        / weights.sum()
    )
